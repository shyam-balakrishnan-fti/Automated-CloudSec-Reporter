...
def generate_review_html(
    ...
    # HTML is identical to stage_reviewer.py with M365 label changes
    ...
    <div style="flex:1">
      <div class="global-ai-input-wrap">
        <textarea class="global-ai-input" id="global-ai-input" rows="1"
          placeholder="Board-wide instruction — e.g. &quot;merge all Entra ID MFA controls into one group&quot; or &quot;split Defender checks by policy type&quot;"></textarea>
...
function init() {{
  groups = PROPOSAL.map((g,i) => ({{
    ...
    affected_resources: g.affected_resources || [], resources_open: false,
  }}));
  unassigned = []; render();
}}
...
function onDrop(e, toGid) {{
  ...
  dragCid = null; dragFrom = null; render();
...
function deleteGroup(gid) {{ const g = groups.find(x=>x.id===gid); if(!g) return; unassigned.push(...g.check_ids); groups = groups.filter(x=>x.id!==gid); render(); }}
function addNewGroup() {{
  const id = "gnew_" + Date.now();
  groups.push({{ id, group_name: "New Group — rename me", rationale: "", check_ids: [], is_merged: false, affected_resources: [], resources_open: false }});
  render();
  setTimeout(() => {{
...
function resetToProposal() {{ if (confirm("Reset all groups back to the AI proposal? Your changes will be lost.")) init(); }}
...
function regroupOne(gid) {{
  ...
  fetch(`http://localhost:${{PORT}}/regroup-one`, {{
    ...
    body: JSON.stringify({{ instruction, target_group: {{ group_name: g.group_name, check_ids: g.check_ids, rationale: g.rationale }}, all_groups: groups.map(x => ({{ group_name: x.group_name, check_ids: x.check_ids, rationale: x.rationale }})) }}),
  }})
  .then(r => r.json())
  .then(data => {{
...
function regroupGlobal() {{
  ...
  fetch(`http://localhost:${{PORT}}/regroup-global`, {{
    method: "POST", headers: {{"Content-Type":"application/json"}},
    body: JSON.stringify({{ instruction, all_groups: groups.map(g => ({{ group_name: g.group_name, check_ids: g.check_ids, rationale: g.rationale }})), unassigned }}),
  }})
  .then(r => r.json())
  .then(data => {{
...
function applyNewGrouping(newGroupsFromServer) {{
  groups = newGroupsFromServer.map((g, i) => {{
    ...
    return {{ id: "g" + i + "_" + Date.now(), group_name: g.group_name, rationale: g.rationale || "", check_ids: g.check_ids, is_merged: g.check_ids.length > 1, affected_resources: resources, resources_open: false }};
  }});
...
function updateStats() {{
  ...
  setText("s-current", groups.length); setText("s-merged", merged); setText("s-standalone", solo); setText("s-unassigned", unassigned.length);
...
function approveGrouping() {{
  ...
  const payload = {{ run_id: RUN_ID, approved_at: new Date().toISOString(), groups: groups.map(g => ({{ group_name: g.group_name.trim(), check_ids: g.check_ids, rationale: g.rationale || "", analyst_note: "" }})) }};
  ...
  .then(r => r.json())
  .then(data => {{
    if (data.ok) {{ setStatus("✓ Approved — pipeline continuing to enrichment and Excel render...", "ok"); document.getElementById("approve-btn").textContent = "✓ Approved"; }}
    else {{ setStatus("✗ Error: " + data.error, "err"); document.getElementById("approve-btn").disabled = false; }}
  }})
