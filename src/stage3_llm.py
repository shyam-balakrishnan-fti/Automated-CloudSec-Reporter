...
- situation_narrative: 2-4 sentences describing WHAT was found and its current state.
  Focus on observable facts: what is misconfigured, how many resources, which accounts.
  If instance_count > 1, include scope language (e.g. "across {instance_count} resources").
  Do NOT describe what could happen if exploited — that belongs in consequence_narrative.
- consequence_narrative: 1-3 sentences on business/security IMPACT if this is exploited.
  Focus on the realistic worst-case outcome: data breach, privilege escalation, outage, etc.
  Do NOT repeat the situation description — start from 'An attacker could...' or 'This could...'.
